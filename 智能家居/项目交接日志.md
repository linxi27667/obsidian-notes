---
created: 2026-05-18
project: ESP32-P4 LVGL 智能家居控制面板
status: 实物主链路基本正常，待三楼层从机真实联调
---
# ESP32-P4 小智 + LVGL 智能家居融合项目实施计划

## 2026-05-21 MQTT 真实链路与火灾告警迭代记录

本轮目标是把主机、三套从机、LVGL 面板、小智唤醒之间的控制链路从“能发命令”推进到“可闭环确认、可同步真实状态、可按传感器触发告警”。当前主项目和一楼从机已通过编译验证，完整实物联调仍需要把三套从机重新烧录后观察 MQTT 报文与 LVGL 状态。

### 已完成的代码改造

- 新增共享协议头 `shared/mqtt_iot_protocol.h`，主机和三套从机共用同一份 MQTT topic、命令、ACK、传感器、心跳结构定义，避免主从协议继续分叉。
- 新增 V2 心跳 `iot_heartbeat_v2_packet_t`，心跳包含协议版本、设备 ID、设备名、MAC、灯/继电器/舵机状态、烟雾 ADC、雨滴 ADC、火警状态、雨滴状态、求助状态、运行时长等字段。
- 修复三套从机 ACK 闭环：
  - ACK 的 `device_id` 从错误的 `0` 改为楼层真实 ID：一楼 `1`、二楼 `2`、三楼 `3`。
  - ACK 的 `command` 从固定 `IOT_CMD_RESPONSE` 改为原始控制命令，主机才能匹配对应设备并更新 LVGL。
  - 从机收到改变状态的命令后会立即补发一次 V2 心跳，即使 ACK 丢失，主机也能通过下一包状态恢复 UI。
- 二楼从机雨滴 ADC 改为可被心跳读取的真实快照。
- 三楼从机烟雾、雨滴、火警、雨滴状态、求助状态改为可被心跳读取的真实快照。
- 修复三楼火焰/烟雾判断方向：烟雾 ADC 现在按 `smoke_mv >= SMOKE_THRESHOLD_FIRE` 触发火警，和配置注释一致。
- 主机 MQTT 服务现在支持解析 V2 心跳并写入 `mqtt_device_model`，同时保留旧心跳兼容解析。
- `mqtt_device_model` 新增火警、雨滴、求助等独立状态，不再把雨滴状态混进传感器数值字段。
- 控制页设备映射修正：
  - 二楼晾衣架舵机 GPIO 索引修正为从机实际使用的 `6`。
  - 三楼天窗拆分为左天窗 `6`、右天窗 `7`、晾衣架 `8`。
  - 舵机类设备开/关发送角度改为 `180/0`，更符合从机实现。
- MCP `get_sensors` 输出改为真实字段：`smoke_mv`、`rain_mv`、`fire_status`、`fire_alarm`、`rain_status`、`help_status`。

### 火灾告警链路

- 新增 `smart_home_alarm_ui`，火警触发时 LVGL 立即显示全屏告警弹窗。
- 弹窗内容包含发生火警的楼层，按钮为“解除警报”。点击后关闭弹窗；如果火警状态没有恢复，后续重复心跳不会反复弹窗，直到火警清除后再允许下一次触发。
- 新增 `smart_home_alarm_bridge.cc`，主机模型检测到火警上升沿后调用：
  - `Application::Alert(...)` 播放现有警告提示音；
  - `WakeWordInvoke("fire_alarm")` 主动唤醒小智。
- 当前“语音播报检测到发生火灾”还不是完整本地 TTS。现阶段是“警告音 + 唤醒小智 + UI 弹窗”。如果要固定播报中文句子，建议后续加本地 OGG 语音资源，或接入小智服务端的主动播报接口。

### 总览页建议

总览页建议优先显示本地真实数据，不建议一开始依赖公网天气 API。原因是智能家居安全和运维判断应优先来自本机与从机真实状态，天气适合作为增强信息。
...
- 每层灯、继电器、舵机状态。
- 二楼雨滴 ADC、雨滴状态。
- 三楼烟雾 ADC、雨滴 ADC、火警、求助状态。
- 主机系统状态：运行时长、可用堆、PSRAM、网络 RSSI、MQTT 重连次数。

天气数据可以第二阶段加入，需要先确定城市、数据源/API Key、离线时显示策略。建议只显示当地天气、温度、湿度、降雨概率，不参与核心告警判断。

### 丝印/品牌图

已把 LVGL 当前代码绘制的顶部品牌区域调大并调整为更接近新图：左侧芯片 `9th` 标识、中文 “AI赋能设计，设计点亮AI！” 和英文 “AI for Design & Design for AI !”。当前是代码绘制，不是把聊天中的图片直接嵌入为位图资源。若要 1:1 使用新丝印，需要提供原始 PNG/SVG 文件，并走资源转换/打包流程。

### 聊天角色替换评估

把聊天界面的小黄人换成自己的卡通角色是可行的，难度取决于目标效果：

- 低难度：保持现有情绪/状态 key 不变，只替换同尺寸透明 PNG/GIF/WebP 素材。主要改资源文件和默认情绪映射。
- 中等难度：为角色补齐完整状态集，例如待机、聆听、思考、说话、困惑、错误、告警等，并适配不同动画时长。
- 高难度：改成骨骼动画、Live2D、复杂交互角色，涉及渲染、资源加载、状态机和性能评估。

需要重点检查和修改的模块：

- `main/assets.cc`：默认资源版本和默认表情资源。
- `main/display/lvgl_display/emoji_collection.*`：表情/动画集合加载逻辑。
- `main/display/lcd_display.cc`：`SetEmotion` 等聊天状态到动画资源的映射。
- 资源目录和资源清单：保持资源名兼容可以显著降低改动量。

推荐路线：先按现有小黄人的状态 key 做一套同尺寸透明角色图，完成静态/轻动画替换；确认性能和内存后再扩展更多动作。

### MQTT 模拟脚本

新增 `tools/mqtt_iot_simulator.py`，用于在没有真实从机或需要快速验证 UI 链路时模拟三套 ESP32-S3 从机。脚本不依赖第三方 Python 包，直接使用标准库实现 MQTT 3.1.1 最小客户端。

常用命令：

```powershell
python tools\mqtt_iot_simulator.py --self-test
python tools\mqtt_iot_simulator.py --verbose
python tools\mqtt_iot_simulator.py --fire-floor 3 --fire-after 3 --clear-fire-after 20 --verbose
python tools\mqtt_iot_simulator.py --floors 2,3 --rain-floor 2 --interval 5 --verbose
```

脚本能力：

- 订阅 `xiaozhi/iot/cmd/broadcast` 和 `xiaozhi/iot/cmd/{floor}`。
- 发布 `announce`、ACK 和 V2 心跳。
- 模拟灯、继电器、舵机状态变更。
- 模拟二楼/三楼雨滴状态。
- 模拟三楼火警，触发主机 LVGL 弹窗和小智唤醒链路。

### 验证与目录约束

- 模拟脚本自检 `python tools\mqtt_iot_simulator.py --self-test` 已通过，确认命令包、V2 心跳包、火警字段、舵机 ACK 打包长度与协议一致。
- 主项目 `idf.py build` 已通过：`xiaozhi.bin` 大小 `0x39bcf0`，app 分区剩余 `0x54310`，约 `8%`。
- 一楼从机已通过编译：`iot_slave_yaohe_Firstfloor.bin` 大小 `0xd9dd0`，app 分区剩余 `0x26230`，约 `15%`。
- 二楼从机已通过编译：`iot_slave_yaohe_Secondfloor.bin` 大小 `0xdba00`，app 分区剩余 `0x24600`，约 `14%`。
- 三楼从机已通过编译：`iot_slave_yaohe_Thirdfloor.bin` 大小 `0xe08c0`，app 分区剩余 `0x1f740`，约 `12%`。
- 编译全部使用项目根目录下唯一的 `build` 目录，其中从机产物分别位于 `build/slave_first`、`build/slave_second`、`build/slave_third`。
- 已确认项目外没有遗留 `E:\MCU\esp32\build` 构建目录。

### 下一步实物联调清单

1. 重新烧录主机、一楼、二楼、三楼固件。
2. 观察 MQTT broker 上的 `xiaozhi/iot/heartbeat/#` 是否每 30 秒出现一次 V2 心跳。
3. 在 LVGL 控制页逐个开关灯、继电器、舵机，确认从机执行、ACK 返回、心跳补偿、LVGL 状态四者一致。
4. 人为触发三楼烟雾/火警阈值，确认 LVGL 弹窗、小智唤醒、解除警报按钮行为。
5. 校准烟雾和雨滴 ADC 阈值，避免误报。
